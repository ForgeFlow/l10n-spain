...
class AccountTax(models.Model):
    ...
    def _l10n_es_edi_verifactu_get_suggested_clave_regimen(
        self, special_regime, forced_verifactu_tax_type=None
    ):
        """
        ...
        if forced_verifactu_tax_type:
            ...
            taxes = taxes.filtered(
                lambda tax: (
                    ...
                    or tax._l10n_es_edi_verifactu_get_tax_type()
                    != forced_verifactu_tax_type
                )
            )

        verifactu_tax_type = (
            forced_verifactu_tax_type or taxes._l10n_es_edi_verifactu_get_tax_type()
        )
        ...
        if VAT and oss_tag and oss_tag.id in taxes.repartition_line_ids.tag_ids.ids:
            ...
            lambda tax: tax.l10n_es_type == "exento"
            and tax.l10n_es_exempt_reason == "E2"
        ):
            # export
            regimen_key = "02"
    ...
    def _l10n_es_edi_verifactu_get_tax_details_functions(self, company):
        def full_filter_invl_to_apply(line):
            return any(
                t != "ignore"
                for t in line.tax_ids.flatten_taxes_hierarchy().mapped("l10n_es_type")
            )
        ...
        def grouping_key_generator(base_line, tax_values):
            ...
            if (
                ...
                recargo_taxes = base_line["taxes"].filtered(
        ...
        def filter_to_apply(base_line, tax_values):
            return (
                ...
                and tax_values["tax_repartition_line"].tax_id.l10n_es_type
                not in ("ignore", "retencion")
            )
