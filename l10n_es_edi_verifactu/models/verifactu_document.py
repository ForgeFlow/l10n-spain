...
class L10nEsEdiVerifactuDocument(models.Model):
    ...
    def _get_last(self, document_type):
        return self.filtered(
            lambda doc: doc.document_type == document_type and doc.json_attachment
        ).sorted()[:1]
    ...
    def _check_record_values(self, vals):
        ...
        if (
            ...
            errors.append(
                _(
                    "A refund with Refund Reason %(refund_reason)s needs a partner.",
        ...
        if len(verifactu_tax_types) > 1:
            name_map = self.env[
                "account.tax"
            ]._l10n_es_edi_verifactu_get_tax_types_name_map()
            human_readable_types = [name_map[t] for t in verifactu_tax_types]
            errors.append(
                _(
                    "We only allow a single Veri*Factu Tax Type (Impuesto) per document: %(types)s.",
        ...
        for record_detail in vals["tax_details"]["tax_details_per_record"].values():
            main_tax_details = [
                tax_detail
                for key, tax_detail in record_detail["tax_details"].items()
                if tax_detail["l10n_es_type"] in main_tax_types
            ]
            if len(main_tax_details) > 1 or any(
                len(tax_detail["group_tax_details"]) > 1
                for tax_detail in main_tax_details
            ):
    ...
    def _create_for_record(self, record_values, previous_record_identifier=None):
        ...
        if record_values["errors"]:
            ...
            if not document_vals.get("errors"):
                document_vals.update(
                    {
                        "chain_index": record_values[
                            "company"
                        ]._l10n_es_edi_verifactu_get_next_chain_index(),
        ...
        if document.json_attachment:
            record_values["documents"].filtered(
                lambda rd: not rd.json_attachment
            ).unlink()
    ...
    def _create_from_record_values_list(self, record_values_list):
        ...
        for company, company_record_values_list in grouped_record_values_list.items():
            ...
            try:
                ...
                lock_error = _(
                    "Someone else is already generating Veri*Factu documents for the same company."
                )
                continue
    ...
    def _render_vals_monetary_amounts(self, vals):
        ...
        for tax_details_per_record in vals["tax_details"][
        ...
        for key, tax_detail in vals["tax_details"]["tax_details"].items():
            ...
            if (
                ...
                and vals["verifactu_tax_type"] == "01"
            ):
                tax_percentage = None
                tax_amount = None
    ...
    def _get_zeep_operation(self, operation):
        ...
        if operation == "registration":
            ...
            zeep_client = (
                client._Client__obj
            )  # get the "real" zeep client from the odoo specific wrapper
            service = zeep_client.bind(wsdl["service"], wsdl["port"])

            def function(*args, **kwargs):
                return zeep_client.create_message(
                    service, wsdl["registration"], *args, **kwargs
                )
    ...
    def _send_batch(self, batch_dict):
        ...
        try:
            ...
            error = _(
                "Error while validating the batch document (before sending):\n%s", error
            )
            errors.append(error)
            _logger.error("%s:\n%s", error, batch_dict)
        except zeep.exceptions.Error as error:
        ...
        info.update(
            {
                "response_csv": res["CSV"]
                if "CSV" in res
                else None,  # noqa: SIM401 - `res` is of type 'zeep.client.SerialProxy'
    ...
    def _send_as_batch(self):
        ...
        incident = any(
            document.create_date > self.env.cr.now() + timedelta(seconds=240)
            for document in self
        )
    ...
    def _send_as_batch_check(self):
        ...
        if company != self.company_id:
            errors.append(
                _("Some of the documents do not belong to the active company.")
            )
