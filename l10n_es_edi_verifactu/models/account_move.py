...
class AccountMove(models.Model):
    ...
    def _l10n_es_edi_verifactu_clave_regimen_selection(self):
        return [
            ...
            (
                "11",
                _("Leasing of business premises"),
            ...
            ),  # Operaciones de actividades incluidas en el Régimen Especial de Agicultura, Ganadería y Pesca (REAGYP)
            ("20_iva", _("Simplified Regime")),  # Régimen simplificado
            # IGIC only
            (
                "17_igic",
                _("Special retailer regime"),
    ...
    def _compute_l10n_es_edi_verifactu_available_clave_regimens(self):
        ...
        for move in self:
            ...
            move.l10n_es_edi_verifactu_available_clave_regimens = (
                available_clave_regimens.get(verifactu_tax_type, False)
            )
    ...
    @api.depends(
        ...
        "l10n_es_edi_verifactu_document_ids.json_attachment",
    )
    ...
    def _l10n_es_edi_verifactu_check(self, cancellation=False):
        ...
        if (
            ...
            not in available_clave_regimens_map.get(verifactu_tax_type, set())
        ):
            errors.append(
                _(
                    "The Veri*Factu Regime Key is not compatible with the Veri*Factu Tax Type."
                )
            )
    ...
    def _l10n_es_edi_verifactu_get_record_values(self, cancellation=False):
        ...
        vals.update(
            {
                ...
                "substituted_document_reversal_document": substituted_move.reversal_move_id.l10n_es_edi_verifactu_document_ids._get_last(
                    "submission"
                ),
                "documents": documents,
        ...
        tax_details_functions = self.env[
            "account.tax"
        ]._l10n_es_edi_verifactu_get_tax_details_functions(company)
        ...
        vals["errors"] = self.env[
            "l10n_es_edi_verifactu.document"
        ]._check_record_values(vals)
        ...
        if (
            ...
            action = self._l10n_es_edi_verifactu_action_go_to_journal_entry(
                substituted_move
            )
        ...
        if (
            ...
            action = self._l10n_es_edi_verifactu_action_go_to_journal_entry(
                substituted_move.reversal_move_id
            )
            raise RedirectWarning(msg, action, _("Go to the journal entry"))
        if (
    ...
    def _l10n_es_edi_verifactu_create_documents(self, cancellation=False):
        ...
        return (
            ...
            .sudo()
            ._create_from_record_values_list(record_values_list)
        )
    ...
    def l10n_es_edi_verifactu_button_send(self):
        invoices_to_send = self.filtered(lambda m: m.l10n_es_edi_verifactu_required)
        invoices_to_send._l10n_es_edi_verifactu_mark_for_next_batch()
