...
class SendSIIWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        account_moves_with_triggers = account_moves.filtered(
            lambda i: (i.sii_invoice_cron_trigger_ids)
        )
        account_moves_without_triggers = account_moves - account_moves_with_triggers
        not_send_without_errors = account_moves_without_triggers.filtered(
            lambda a: a.aeat_state == "not_sent" and not a.aeat_send_failed
        )
        with_errors = account_moves_without_triggers.filtered(
            lambda a: a.aeat_send_failed
        )
        modified = account_moves_without_triggers.filtered(
            lambda a: a.aeat_state in ["sent_modified", "cancelled_modified"]
        )
        res.update(
            {
                "sending_number": len(account_moves_with_triggers),
