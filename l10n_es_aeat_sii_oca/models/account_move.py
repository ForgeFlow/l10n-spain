...
class AccountMove(models.Model):
    ...
    sii_invoice_cron_trigger_ids = fields.Many2many(
        comodel_name="ir.cron.trigger",
        column1="sii_invoice_id",
        column2="trigger_id",
    ...
    def _get_sii_triggers_field_name(self):
        return "sii_invoice_cron_trigger_ids"
    ...
    def cancel_sii(self):
        ...
        for invoice in invoices:
            ...
            invoice.sudo().sii_invoice_cron_trigger_ids |= trigger
            invoice.write({"sii_send_date": False, "sii_cancel_date": sii_sending_time})
    ...
    def _send_to_sii(self):
        ...
        cancel_documents = all_cancel_documents = self.search(
            [
                ...
                ("sii_cancel_date", "<=", fields.Datetime.now()),
            ]
        )
        if documents or cancel_documents:
            cron_trigger_obj = self.env["ir.cron.trigger"].sudo()
            sii_send_cron = self.env.ref("l10n_es_aeat_sii_oca.invoice_send_to_sii")
            ...
            for document in remaining_documents + remaining_cancel_documents:
                ...
                document.sudo().sii_invoice_cron_trigger_ids |= trigger
