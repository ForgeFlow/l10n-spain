...
class TestVerifactuSealOnActivation(TestVerifactuCommon):
    ...
    def test_activation_warns_about_sealing(self):
        journal = self.env["account.journal"].search(
            [("company_id", "=", self.company.id), ("type", "=", "sale")], limit=1
        )
        self.company.verifactu_enabled = False
        journal.restrict_mode_hash_table = False
        self._create_invoice("SEAL/2026/00001", "2026-01-10").action_post()
        company = self.company.new(origin=self.company)
        company.verifactu_enabled = True
        warning = company._onchange_verifactu_enabled()["warning"]
